...
def conv2d_forward(x, w, b, pad, stride):
    ...
    #                                                                     #
    #######################################################################
    num_of_filters=w.shape[0]
    filter_height=w.shape[1]
    filter_width=w.shape[2]
    # print("fw",filter_width)
    channels=w.shape[3]
    height=x.shape[1]
    width=x.shape[2]
    N=x.shape[0]
    ...
    # print(x.shape)
    out=np.empty(shape=(N,new_height, new_width, num_of_filters))
        ...
        for f in range(num_of_filters):
            for h in range(new_height):
                ...
                for wi in range(new_width):
                    wis=wi*stride
                    # print("wis-", wis)
                    window=x[i, hs:hs+filter_height, wis:wis+filter_width, :]
                    # print(window.shape)
                    # print(w[f].shape)
                    out[i, h, wi, f]=np.sum(window*w[f])+b[f]
