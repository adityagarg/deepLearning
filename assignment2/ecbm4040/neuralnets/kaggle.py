...
def my_LeNet(input_x, input_y,
	...
	conv_layer_0_1= conv_layer(input_x=conv_layer_0.output(),
	...
	batch_norm_layer_0=tf.layers.batch_normalization(pooling_layer_0.output())



	conv_layer_1= conv_layer(input_x=batch_norm_layer_0,
							  in_channel=conv_featmap[1],
							  out_channel=conv_featmap[2],
							  kernel_shape=conv_kernel_size[2],
							  rand_seed=seed, index=2)

	conv_layer_1_1= conv_layer(input_x=conv_layer_1.output(),
	...
	batch_norm_layer_1=tf.layers.batch_normalization(pooling_layer_1.output())

	conv_layer_2= conv_layer(input_x=batch_norm_layer_1,
							  in_channel=conv_featmap[3],
							  out_channel=conv_featmap[4],
							  kernel_shape=conv_kernel_size[4],
							  rand_seed=seed, index=4)

	conv_layer_2_1= conv_layer(input_x=conv_layer_2.output(),
	...
	batch_norm_layer_2=tf.layers.batch_normalization(pooling_layer_2.output())

	conv_layer_3= conv_layer(input_x=batch_norm_layer_2,
							  in_channel=conv_featmap[5],
							  out_channel=conv_featmap[6],
							  kernel_shape=conv_kernel_size[6],
							  rand_seed=seed, index=6)

	conv_layer_3_1= conv_layer(input_x=conv_layer_3.output(),
							  in_channel=conv_featmap[6],
							  out_channel=conv_featmap[7],
							  kernel_shape=conv_kernel_size[7],
							  rand_seed=seed, index=7)

	pooling_layer_3= max_pooling_layer(input_x=conv_layer_3_1.output(),
										k_size=pooling_size[3],
										padding="VALID")

	batch_norm_layer_3=tf.layers.batch_normalization(pooling_layer_3.output())


	# flatten

	# pool_shape = pooling_layer_1.output().get_shape()
	pool_shape = batch_norm_layer_3.get_shape()
	img_vector_length = pool_shape[1].value * pool_shape[2].value * pool_shape[3].value
	flatten = tf.reshape(batch_norm_layer_3, shape=[-1, img_vector_length])
	...
	# print("fc leayer 1", fc_layer_1.output().shape)

	# saving the parameters for l2_norm loss
	conv_w = [conv_layer_0.weight, conv_layer_0_1.weight, conv_layer_1.weight, conv_layer_1_1.weight, conv_layer_2.weight, conv_layer_2_1.weight, conv_layer_3.weight, conv_layer_3_1.weight]
...
def my_LeNet_small(input_x, input_y,
		  img_len=32, channel_num=3, output_size=5,
		  conv_featmap=[6, 16], fc_units=[84],
		  conv_kernel_size=[5, 5], pooling_size=[2, 2],
		  l2_norm=0.01, seed=235):
	...
	dropout_layer_0=tf.layers.dropout(fc_layer_0.output(), rate=0.25)

	fc_layer_1 = fc_layer(input_x=dropout_layer_0,
						  in_size=fc_units[0],
						  out_size=output_size,
						  rand_seed=seed,
						  activation_function=None,
...
def my_LeNet_final(input_x, input_y,
		  img_len=32, channel_num=3, output_size=5,
		  conv_featmap=[6, 16], fc_units=[84],
		  conv_kernel_size=[5, 5], pooling_size=[2, 2],
		  l2_norm=0.01, seed=235):
	...
	# pool_shape = pooling_layer_1.output().get_shape()
	pool_shape = batch_norm_layer_1.get_shape()
	img_vector_length = pool_shape[1].value * pool_shape[2].value * pool_shape[3].value
	flatten = tf.reshape(batch_norm_layer_1, shape=[-1, img_vector_length])
...
def my_training_task(X_train, y_train, X_val, y_val, 
			 conv_featmap=[6],
			 fc_units=[84],
			 conv_kernel_size=[5],
			 pooling_size=[2],
			 ...
			 batch_size=245,
			 verbose=False,
			 pre_trained_model=None, output_size=5, perc_augment=0.2, modelName="my_LeNet"):
	print("Building my LeNet. Parameters: ")
	print("conv_featmap={}".format(conv_featmap))
	print("fc_units={}".format(fc_units))
	print("conv_kernel_size={}".format(conv_kernel_size))
	...
	with tf.Session() as sess:
		...
		for epc in range(epoch):
			...
			for itr in range(iters):
				...
				idx=np.random.choice(training_batch_x.shape[0], size=int(training_batch_x.shape[0]*perc_augment), replace=False)
				training_batch_x_aug=reshape_data(training_batch_x[idx], training_batch_y[idx])
				# print(training_batch_x_aug.shape)
				training_batch_x[idx]=training_batch_x_aug

				# print("training batch shape", training_batch_x.shape)
