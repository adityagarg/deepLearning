    ...
    def train(self, model, X_train, y_train, X_valid, y_valid,
        ...
        for e in range(num_epoch):
            ...
            for i in range(num_batch):
                ...
                # print(model.params[list(model.params.keys())[1]][:1, :1])
...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            cache[k]=0.9*cache[k]+0.1*((grads[k])**2)
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            mhat=momentums[k]/(1.0-beta1**t)
            vhat=velocitys[k]/(1.0-beta2**t)

            params[k]=params[k]-learning_rate*mhat/(np.sqrt(vhat)+eps)
